...
for i in range(len(geojson_list)):
    ...
    if type == 'Polygon':
        ...
        record = ee.Feature(ee.Geometry.Polygon(geojson_list[i]['coordinates'], proj=shp_proj, geodesic=False)).set(
            'class', 'water')
    elif type == 'MultiPolygon':
        # continue
        record = ee.Feature(ee.Geometry.MultiPolygon(geojson_list[i]['coordinates'], proj=shp_proj, geodesic=False),
                            {'class': 'water'})
    elif type == 'Point':
        record = ee.Feature(ee.Geometry.Point(geojson_list[i]['coordinates'], proj=shp_proj, geodesic=False),
                            {'class': 'water'})
...
# method 1 通过dataid进行过滤,但可能因为数据版本原因找不到对应影像， 如 dataid= "LT51340362005235BJC00"，gee中LT51340362005235BJC01,所以Pass
...
# method 2 通过标注矢量文件范围，和时间,但标注的影像可能有云，所以这个方法也不合适，Pass
...
# method 3 通过标注矢量文件范围(metadata properties: 'WRS_PATH': 134,  'WRS_ROW': 36)，和时间
path = 134
row = 36
from datetime import datetime, timedelta

feat_date = "2005-08-23"
...
# read metadata and profile of the image for later image storage and download
scene_id = image.get('LANDSAT_SCENE_ID').getInfo()
bandNames = image.bandNames().getInfo()
profile = image.projection().getInfo()
crs = profile['crs']
crs_transform = profile['transform']

# 第三步，计算各种water index, 将指数特征波段加入到image中
...
NDWI = image.expression('(NIR - SWIR5)/(NIR + SWIR5)', {'NIR': image.select('B4'), 'SWIR5': image.select('B5')}).rename('NDWI')
MNDWI = image.expression('(GREEN - SWIR5)/(GREEN + SWIR5)', {'GREEN': image.select('B2'), 'SWIR5': image.select('B5')}).rename('MNDWI')
EWI = image.expression('(GREEN - NIR - SWIR5)/(GREEN + NIR + SWIR5)',{'GREEN': image.select('B2'), 'NIR': image.select('B4'), 'SWIR5': image.select('B5')}).rename('EWI')
NEW = image.expression('(BLUE - SWIR7)/(BLUE + SWIR7)',{'BLUE': image.select('B1'), 'SWIR7': image.select('B7')}).rename('NEW')
NDWI_B = image.expression('(BLUE - NIR)/(BLUE+NIR)', {'BLUE': image.select('B1'), 'NIR': image.select('B4')}).rename('NDWI_B')
AWElnsh = image.expression('4*(GREEN-SWIR5)-(0.25*NIR+2.75*SWIR7)',{'GREEN': image.select('B2'), 'SWIR5': image.select('B5'), 'NIR': image.select('B4'),'SWIR7': image.select('B7')}).rename('AWElnsh')
